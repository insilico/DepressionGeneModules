...
group.genes <- read.csv("wgcna.0.2.group.new.csv", stringsAsFactors = F, row.names = 1)
...
# error with rnaseq=T
#res <- gsva(expr=expr, gset.idx.list=c7_genes_sets$genesets, method="ssgsea", rnaseq=T, verbose=T)
res <- gsva(expr=expr, gset.idx.list=c7_genes_sets$genesets, method="ssgsea", verbose=T) 
